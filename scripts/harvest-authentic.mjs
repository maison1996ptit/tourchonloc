/**
 * @file scripts/harvest-authentic.mjs
 * @description Sourcing 80 100% authentic, high-quality landmark images using Wikipedia / Wikimedia Commons.
 * @version 2.0.0
...
async function fetchWikipediaImage(searchQuery, lang = 'vi') {
  ...
  try {
    const searchRes = await fetch(searchUrl, {
      headers: { 'User-Agent': 'TravelAppBot/2.0 (admin@travelapp.com; contact@travelapp.com; Mozilla/5.0)' }
    });
...
async function processImage(id, name, url) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) TravelApp/2.0' }
    });
    ...
    clearTimeout(timeoutId);
    console.error(`[ FAILED ] ${id.toUpperCase()} -> ${name}: ${e.message}`);
    // Clean up temp file
    try {
      await fs.unlink(tempPath);
    } catch {}
...
async function main() {
  console.log("==================================================================");
  console.log(" STARTING AUTHENTIC IMAGE HARVESTING: 80 DESTINATIONS");
  console.log("==================================================================");
  ...
  for (let i = 0; i < DATASET.length; i += CONFIG.CONCURRENCY_LIMIT) {
    const batch = DATASET.slice(i, i + CONFIG.CONCURRENCY_LIMIT);
    
    await Promise.all(batch.map(async (item) => {
      console.log(`Resolving URL for: ${item.id.toUpperCase()} (${item.name})...`);
  ...
  // Write audit report
  let report = `# AUTHENTIC LANDMARK IMAGES HARVEST REPORT\n\n`;
