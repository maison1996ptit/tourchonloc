...
export async function parseTourPDF(formData: FormData) {
  if (!(await isEditor())) throw new Error('Unauthorized');

  const file = formData.get('file') as File;
  if (!file) throw new Error('Không tìm thấy tệp tải lên');
  ...
  try {
    ...
    throw new Error('Không thể đọc nội dung file PDF. Vui lòng kiểm tra định dạng file.');
  }

  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    throw new Error('GEMINI_API_KEY chưa được cấu hình trong file .env. Vui lòng thêm khóa này để dùng chức năng AI.');
  }
...
---
    ...
    if (!textResponse) {
      throw new Error(lastError?.message || 'Không nhận được phản hồi từ AI');
    }
    ...
    throw new Error('Lỗi khi gọi API Gemini để phân tích dữ liệu: ' + (err as Error).message);
  }
}
