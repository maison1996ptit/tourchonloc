...
function CentralizedBlogsContent() {
  ...
  const handleDelete = async (id: string, title: string) => {
    if (!isAdmin) {
      alert('Bạn không có quyền thực hiện hành động này. Chỉ Admin mới được xóa bài viết.');
      return;
    }
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <div>
          ...
          <p style={{ fontSize: '0.9rem', color: '#64748b', margin: 0 }}>
            Hệ thống quản trị tập trung cho bài viết tin tức, cẩm nang du lịch và ghi nhớ bán hàng.
          </p>
        </div>
        {isAdmin ? (
      ...
      {loading ? (
        ...
        <div className={styles.loading}>Không tìm thấy bài viết nào.</div>
      ) : (
        <>
                  ...
                  <tr key={guide.id}>
                    ...
                    <td>
                      {guide.country ? (
                        ...
                        <span style={{ color: '#94a3b8', fontSize: '0.85rem' }}>Không chỉ định</span>
                      )}
                      {guide.city && (
                    ...
                    <td>
                      <span className={`${styles.statusTag} ${styles[guide.status.toLowerCase()] || styles.draft}`}>
                        {guide.status === 'Published' ? 'Công khai' : guide.status === 'Draft' ? 'Bản nháp' : 'Hẹn giờ'}
                      </span>
                    </td>
                    <td className={styles.actions}>
                      <Link href={`/admin/blogs/${guide.id}/edit`} className={styles.editBtn}>
                        {isAdmin ? 'Sửa' : 'Xem'}
                      </Link>
                      {isAdmin && (
