...
interface VisaPageClientProps {
  ...
  initialStats: VisaStats | null;
}

export default function VisaPageClient({ initialVisas, initialStats }: VisaPageClientProps) {
  const { t } = useLanguage();
  const [visas, setVisas] = useState<VisaService[]>(initialVisas);
  const [stats, setStats] = useState<VisaStats | null>(initialStats);
