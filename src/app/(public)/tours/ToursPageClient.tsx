...
function ToursList({ initialTours }: ToursPageClientProps) {
  const { t } = useLanguage();
...
export default function ToursPageClient({ initialTours }: ToursPageClientProps) {
  return (
    <Suspense fallback={<div style={{ textAlign: 'center', padding: '100px 20px', color: '#64748b' }}>Đang tải danh sách tour...</div>}>
      <ToursList initialTours={initialTours} />
    </Suspense>
  );
}
