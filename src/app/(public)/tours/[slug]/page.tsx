      ...
      if (typeof slug === 'string') {
        ...
        if (data) {
          ...
          const related = allTours.filter(tData => 
            ...
            (tData.destination === data.destination || tData.category === data.category)
          ).slice(0, 3);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (slider.scrollLeft >= maxScroll - 5) {
        ...
        // Scroll right by 1 card slot
        // 4 items are displayed in the viewport, so each card represents clientWidth / 4.
        const cardWidth = slider.clientWidth / 4;
        slider.scrollBy({ left: cardWidth, behavior: 'smooth' });
      }
    }, 3500); // comfortable reading/scanning speed
